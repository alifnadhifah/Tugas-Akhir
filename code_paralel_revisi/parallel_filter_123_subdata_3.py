...
sub_data_item_path_3 =  f"{base_item_path}sub-data_item_final_3.csv"
sub_data_item_3 =  pd.read_csv(sub_data_item_path_3)

base_user_path = "E:/TA ALIF/Revisi/sub-data_final/"
sub_data_user_path_3 =  f"{base_user_path}sub-data_user_final_3.csv"
sub_data_user_3 =  pd.read_csv(sub_data_user_path_3)

# Tentukan path direktori Drive
base_rating_path = "E:/TA ALIF/Revisi/sub-data_final/"
sub_data_rating_path_3 =  f"{base_rating_path}sub-data_rating_final_3.csv"
sub_data_rating_3 =  pd.read_csv(sub_data_rating_path_3)

# Tentukan path direktori Drive
base_cluster_path = "E:/TA ALIF/Revisi/sub-data_final/"
sub_data_cluster_path_3 =  f"{base_cluster_path}sub-data_cluster_final_3.csv"
sub_data_cluster_3 =  pd.read_csv(sub_data_cluster_path_3)
...
# Preroses synopsis
sub_data_cluster_3['cleaned_synopsis'] = sub_data_cluster_3['Anime_Synopsis'].apply(preprocess)
...
print(f"Item ID {id1}:")
print("Anime Synopsis:", sub_data_cluster_3.loc[id1, 'Anime_Synopsis'])
print("Cleaned Synopsis:", sub_data_cluster_3.loc[id1, 'cleaned_synopsis'])
print()

print(f"Item ID {id2}:")
print("Anime Synopsis:", sub_data_cluster_3.loc[id2, 'Anime_Synopsis'])
print("Cleaned Synopsis:", sub_data_cluster_3.loc[id2, 'cleaned_synopsis'])

# Menggunakan TfidfVectorizer untuk menghitung TF-IDF
tfidf_3 = TfidfVectorizer(stop_words='english')
tfidf_matrix_3 = tfidf_3.fit_transform(sub_data_cluster_3['cleaned_synopsis'])
tfidf_matrix_normalized_3 = normalize(tfidf_matrix_3)
...
anime_ids_3 = sub_data_cluster_3.index

# Membuat DataFrame dari cosine similarity
cosine_sim_df_3 = pd.DataFrame(cosine_sim_3, index=anime_ids_3, columns=anime_ids_3)


## ITEM BASED FILTERING SUB DATA 3
# Convert the DataFrame into Surprise's format
reader_3 = Reader(rating_scale=(sub_data_rating_3['Rating_by_User'].min(), sub_data_rating_3['Rating_by_User'].max()))
data_3 = Dataset.load_from_df(sub_data_rating_3[['User_Id', 'Anime_Id', 'Rating_by_User']], reader_3)

# Split the data into training and test sets
trainset_3, testset_3 = surprise_train_test_split(data_3, test_size=0.2, random_state=42)

# Instantiate the SVD model
svd_3 = SVD()

# Train the SVD model
svd_3.fit(trainset_3)

# Predict ratings for the testset
predictions_3 = svd_3.test(testset_3)

# Mengonversi predictions ke DataFrame
predictions_df_3 = pd.DataFrame(predictions_3, columns=['User_Id', 'Anime_Id', 'Rating_by_User', 'Predicted_Rating', 'Details'])
...
recommendation_folder1_sd3 = 'E:/TA ALIF/Revisi/new_get_recommendations_r1/sub-data_3'  # Ganti dengan path folder yang sesuai

# Create the directory if it doesn't exist
os.makedirs(recommendation_folder1_sd3, exist_ok=True)

recommender1_sd3 = HybridRecommender(sub_data_cluster_3, sub_data_rating_3, cosine_sim_3, svd_3, recommendation_folder1_sd3)
query_user_id1_sd3 = sub_data_user_3['User_Id'].sample(n=1).iloc[0]
query_item_id1_sd3 = sub_data_cluster_3['Anime_Id'].sample(n=1).iloc[0]
print(query_user_id1_sd3, query_item_id1_sd3)
top_10_recommendations1_sd3 = recommender1_sd3.get_top_N_recommendations(user_id=query_user_id1_sd3, item_id=query_item_id1_sd3, N=10)
print(top_10_recommendations1_sd3)
...
def getUserRecommendation(user_id):
    ...
    file_path = os.path.join(recommendation_folder1_sd3, file_name)
    ...
    for item_id in sub_data_cluster_3['Anime_Id']:
        top_10_recommendations = recommender1_sd3.get_top_N_recommendations(user_id=user_id, item_id=item_id, N=10)
...
# avg_precision1_sd3, avg_recall1_sd3, avg_f1_score1_sd3 = recommender1_sd3.evaluate_model(n=10)
# print(f"Avg Precision: {avg_precision1_sd3}, Avg Recall: {avg_recall1_sd3}, Avg F1 Score: {avg_f1_score1_sd3}")
# output_eval = "E:/TA ALIF/Revisi/hasil_evaluasi_baru/sub-data_3/hasil_eval_r1.txt"

# # Create the directory if it doesn't exist
# os.makedirs(os.path.dirname(output_eval), exist_ok=True)

# with open(output_eval, 'w') as f:
#     f.write(f"Avg Precision: {avg_precision1_sd3}, Avg Recall: {avg_recall1_sd3}, Avg F1 Score: {avg_f1_score1_sd3}")