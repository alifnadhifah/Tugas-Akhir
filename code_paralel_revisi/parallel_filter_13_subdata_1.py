...
class HybridRecommender2:
    ...
    def evaluate_model(self, n):
        ...
        avg_f2_score_2 = total_f2_score / count

        return avg_precision_2, avg_recall_2, avg_f2_score_2
        
recommendation_folder2_sd1 = 'E:/TA ALIF/Revisi/new_get_recommendations_r2/sub-data_1'  # Ganti dengan path folder yang sesuai

# Create the directory if it doesn't exist
os.makedirs(recommendation_folder2_sd1, exist_ok=True)

recommender2_sd1 = HybridRecommender2(sub_data_cluster_1, sub_data_rating_1, svd_1, recommendation_folder2_sd1)
...
def getUserRecommendation(user_id):
    ...
    file_path = os.path.join(recommendation_folder2_sd1, file_name)
    ...
    for item_id in sub_data_cluster_1['Anime_Id']:
        top_10_recommendations = recommender2_sd1.get_top_N_recommendations(user_id=user_id, item_id=item_id, N=10)
...
# avg_precision2_sd1, avg_recall2_sd1, avg_f2_score2_sd1 = recommender2_sd1.evaluate_model(n=10)
# print(f"Avg Precision: {avg_precision2_sd1}, Avg Recall: {avg_recall2_sd1}, Avg F1 Score: {avg_f2_score2_sd1}")
# output_eval = "E:/TA ALIF/Revisi/hasil_evaluasi_baru/sub-data_1/hasil_eval_r2.txt"
# # Create the directory if it doesn't exist
# os.makedirs(os.path.dirname(output_eval), exist_ok=True)
# with open(output_eval, 'w') as f:
#     f.write(f"Avg Precision: {avg_precision2_sd1}, Avg Recall: {avg_recall2_sd1}, Avg F1 Score: {avg_f2_score2_sd1}")