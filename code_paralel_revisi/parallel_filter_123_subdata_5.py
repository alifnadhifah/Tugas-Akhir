...
sub_data_item_path_5 =  f"{base_item_path}sub-data_item_final_5.csv"
sub_data_item_5 =  pd.read_csv(sub_data_item_path_5)

base_user_path = "E:/TA ALIF/Revisi/sub-data_final/"
sub_data_user_path_5 =  f"{base_user_path}sub-data_user_final_5.csv"
sub_data_user_5 =  pd.read_csv(sub_data_user_path_5)

# Tentukan path direktori Drive
base_rating_path = "E:/TA ALIF/Revisi/sub-data_final/"
sub_data_rating_path_5 =  f"{base_rating_path}sub-data_rating_final_5.csv"
sub_data_rating_5 =  pd.read_csv(sub_data_rating_path_5)

# Tentukan path direktori Drive
base_cluster_path = "E:/TA ALIF/Revisi/sub-data_final/"
sub_data_cluster_path_5 =  f"{base_cluster_path}sub-data_cluster_final_5.csv"
sub_data_cluster_5 =  pd.read_csv(sub_data_cluster_path_5)
...
# Preroses synopsis
sub_data_cluster_5['cleaned_synopsis'] = sub_data_cluster_5['Anime_Synopsis'].apply(preprocess)
...
print(f"Item ID {id1}:")
print("Anime Synopsis:", sub_data_cluster_5.loc[id1, 'Anime_Synopsis'])
print("Cleaned Synopsis:", sub_data_cluster_5.loc[id1, 'cleaned_synopsis'])
print()

print(f"Item ID {id2}:")
print("Anime Synopsis:", sub_data_cluster_5.loc[id2, 'Anime_Synopsis'])
print("Cleaned Synopsis:", sub_data_cluster_5.loc[id2, 'cleaned_synopsis'])

# Menggunakan TfidfVectorizer untuk menghitung TF-IDF
tfidf_5 = TfidfVectorizer(stop_words='english')
tfidf_matrix_5 = tfidf_5.fit_transform(sub_data_cluster_5['cleaned_synopsis'])
tfidf_matrix_normalized_5 = normalize(tfidf_matrix_5)

# Menghitung kemiripan kosinus
cosine_sim_5 = linear_kernel(tfidf_matrix_5, tfidf_matrix_5)

# Misalkan 'anime_id' adalah index dari df
anime_ids_5 = sub_data_cluster_5.index

# Membuat DataFrame dari cosine similarity
cosine_sim_df_5 = pd.DataFrame(cosine_sim_5, index=anime_ids_5, columns=anime_ids_5)


## ITEM BASED FILTERING SUB DATA 3
# Convert the DataFrame into Surprise's format
reader_5 = Reader(rating_scale=(sub_data_rating_5['Rating_by_User'].min(), sub_data_rating_5['Rating_by_User'].max()))
data_5 = Dataset.load_from_df(sub_data_rating_5[['User_Id', 'Anime_Id', 'Rating_by_User']], reader_5)

# Split the data into training and test sets
trainset_5, testset_5 = surprise_train_test_split(data_5, test_size=0.2, random_state=42)

# Instantiate the SVD model
svd_5 = SVD()

# Train the SVD model
svd_5.fit(trainset_5)

# Predict ratings for the testset
predictions_5 = svd_5.test(testset_5)

# Mengonversi predictions ke DataFrame
predictions_df_5 = pd.DataFrame(predictions_5, columns=['User_Id', 'Anime_Id', 'Rating_by_User', 'Predicted_Rating', 'Details'])

# Menampilkan tabel predictions
predictions_df_5.head()
...
recommendation_folder1_sd5 = 'E:/TA ALIF/Revisi/new_get_recommendations_r1/sub-data_5'  # Ganti dengan path folder yang sesuai

# Create the directory if it doesn't exist
os.makedirs(recommendation_folder1_sd5, exist_ok=True)

recommender1_sd5 = HybridRecommender(sub_data_cluster_5, sub_data_rating_5, cosine_sim_5, svd_5, recommendation_folder1_sd5)
query_user_id1_sd5 = sub_data_user_5['User_Id'].sample(n=1).iloc[0]
query_item_id1_sd5 = sub_data_cluster_5['Anime_Id'].sample(n=1).iloc[0]
print(query_user_id1_sd5, query_item_id1_sd5)
top_10_recommendations1_sd5 = recommender1_sd5.get_top_N_recommendations(user_id=query_user_id1_sd5, item_id=query_item_id1_sd5, N=10)
print(top_10_recommendations1_sd5)
...
def getUserRecommendation(user_id):
    ...
    file_path = os.path.join(recommendation_folder1_sd5, file_name)
    ...
    for item_id in sub_data_cluster_5['Anime_Id']:
        top_10_recommendations = recommender1_sd5.get_top_N_recommendations(user_id=user_id, item_id=item_id, N=10)
...
# avg_precision1_sd5, avg_recall1_sd5, avg_f1_score1_sd5 = recommender1_sd5.evaluate_model(n=10)
# print(f"Avg Precision: {avg_precision1_sd5}, Avg Recall: {avg_recall1_sd5}, Avg F1 Score: {avg_f1_score1_sd5}")
# output_eval = "E:/TA ALIF/Revisi/hasil_evaluasi_baru/sub-data_5/hasil_eval_r1.txt"
# # Create the directory if it doesn't exist
# os.makedirs(os.path.dirname(output_eval), exist_ok=True)
# with open(output_eval, 'w') as f:
#     f.write(f"Avg Precision: {avg_precision1_sd5}, Avg Recall: {avg_recall1_sd5}, Avg F1 Score: {avg_f1_score1_sd5}")